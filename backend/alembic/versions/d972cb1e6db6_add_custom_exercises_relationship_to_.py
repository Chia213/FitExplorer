"""Add custom_exercises relationship to User model

Revision ID: d972cb1e6db6
Revises: fe97a604b853
Create Date: 2025-03-05 17:44:51.105228
...
revision: str = 'd972cb1e6db6'
down_revision: Union[str, None] = 'fe97a604b853'
...
def downgrade() -> None:
    ...
    op.create_table('workouts',
                    sa.Column('id', sa.INTEGER(), server_default=sa.text(
                        "nextval('workouts_id_seq'::regclass)"), autoincrement=True, nullable=False),
    ...
    op.create_table('user_preferences',
                    ...
                    sa.Column('user_id', sa.INTEGER(),
                              autoincrement=False, nullable=True),
                    sa.Column('weight_unit', sa.VARCHAR(),
                              autoincrement=False, nullable=True),
                    sa.Column('goal_weight', sa.DOUBLE_PRECISION(
                        precision=53), autoincrement=False, nullable=True),
                    sa.Column('email_notifications', sa.BOOLEAN(),
                              autoincrement=False, nullable=True),
                    sa.ForeignKeyConstraint(
                        ['user_id'], ['users.id'], name='user_preferences_user_id_fkey'),
                    sa.PrimaryKeyConstraint(
                        'id', name='user_preferences_pkey'),
                    sa.UniqueConstraint(
                        'user_id', name='user_preferences_user_id_key')
                    )
    op.create_index('ix_user_preferences_id',
                    'user_preferences', ['id'], unique=False)
